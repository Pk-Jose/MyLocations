...
class MapViewController: UIViewController {
    ...
    // MARK: - Actions
    @IBAction func showUser() { // zooms in to users location
        let region = MKCoordinateRegion(center: mapView.userLocation.coordinate, latitudinalMeters: 1000, longitudinalMeters: 1000)
        mapView.setRegion(mapView.regionThatFits(region),animated: true)
  }
    
    @IBAction func showLocations() { // shows saved locations on map
        let theRegion = region(for: locations)
        mapView.setRegion(theRegion, animated: true)
    }
    
    // MARK: - Helper methods
    func updateLocations() { // updates location
    ...
    func region(for annotations: [MKAnnotation]) -> MKCoordinateRegion { // calculates a region where saved locations
        let region: MKCoordinateRegion                                   // are saved and zooms in
